...
class _MealDetailScreenState extends State<MealDetailScreen> {
  ...
  void _handleSaveMeal() async {
    ...
    final mealData = {
      ...
      if (widget.imageFile != null) 'image_path': widget.imageFile!.path,
    };

    final bool success;
    if (widget.mealId != null) {
      success = await ApiService.updateMeal(widget.mealId!, mealData);
    } else {
      success = await ApiService.saveMeal(mealData);
    }
    if (!mounted) return;
  ...
  void _handleSaveChanges() {
    ...
    final updatedData = {
      ...
      'date': widget.date,
      'image': widget.image,
      'confidence': _foodLabelController.text,
      'calorieRange': _calorieRangeController.text,
      'calorieCategory': _calorieCategoryController.text,
      'mealType': _selectedMealType,
    };

    // TODO: Save to backend / local storage
    // For now just print and show message
    print('📝 Saving changes: $updatedData');

    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('${widget.name} updated successfully!'),
        backgroundColor: AppColors.sageGreen,
      ),
    );
            ...
            actions: [
              ...
              TextButton(
                onPressed: () {
                  ...
                  // TODO: Delete from backend / local storage
                  print('🗑️ Deleting: ${widget.name}');

                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text('${widget.name} deleted!'),
                      backgroundColor: AppColors.terracotta,
                    ),
                  );
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          _buildReadOnlyField(_selectedMealType),
        ],
      ),
    );
  }
